...
const PILL_COLORS = {
  'needs invoice': { bg: '#f59e0b', text: '#fff' },
...
export function DistributionTab({ distributions, orders, inventory, onNewDistribution, onGenerateInvoice, onInvoiceAll, onEdit, onDelete, onDistributeAll, pendingTodayOrders }) {
  ...
  // Classify each distribution for filtering
  const classify = (d) => {
  ...
  // Sort: pending invoice first, then no-invoice kitchens, then already invoiced
  const sorted = [...filtered].sort((a, b) => {
    const rank = (d) => {
      if (classify(d) === 'needs invoice') return 0
      if (classify(d) === 'no invoice')   return 1
      return 2
    }
      ...
      {distributions.length === 0 ? (
        ...
        <>
          ...
          {printAllOpen && (
            <div style={{ marginBottom: 16 }} id="dist-sheet-all">
              <div className="inv-preview-controls">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn btn-sm btn-ghost" onClick={() => setPrintAllOpen(false)}>
                    Close
                  </button>
          ...
          {sorted.map(dist => {
            ...
            const borderColor = needsInvoice ? '#f59e0b'
              : isInvoiced     ? '#16a34a'
              : 'var(--border)'
            ...
            return (
              <div key={dist.id}>
                <div className="card" style={{ borderLeft: `3px solid ${borderColor}` }}>
                    ...
                    <div>
                      ...
                      {linkedOrder && (
                        <div style={{ fontSize: 11, color: 'var(--sub)', marginTop: 2 }}>
                          from order · {fmtDate(linkedOrder.date)}
                          {linkedOrder.notes ? ` · ${linkedOrder.notes}` : ''}
                        </div>
                      )}
                    </div>
                  </div>

                  {/* Item breakdown — one row per product */}
                  <div style={{ margin: '8px 0 4px', display: 'flex', flexDirection: 'column', gap: 3 }}>
                    {dist.items.map(item => {
                  ...
                  {/* Actions — Generate Invoice is primary when needed */}
                  <div className="row-actions">
                    {needsInvoice && (
                      <button className="btn btn-sm btn-success" onClick={() => onGenerateInvoice(dist.id)}>
                        Generate Invoice
                      </button>
                    )}
                    {!isInvoiced && (
                      <button className="btn btn-sm btn-ghost" onClick={() => onEdit(dist.id)}>
                        Edit
                      </button>
                    )}
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (opening) {
                          setTimeout(() => {
                            document.getElementById(`dist-sheet-${dist.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
                          }, 80)
                        }
                      }}
                    >
                      🖨 {isPrinting ? 'Hide Sheet' : 'Print Sheet'}
                    </button>
                ...
                {/* Print sheet */}
                {isPrinting && (
                  <div style={{ marginTop: 8, marginBottom: 16 }} id={`dist-sheet-${dist.id}`}>
                    <div className="inv-preview-controls">
                      <span style={{ fontSize: 13, fontWeight: 600 }}>
                        {kioskLabel} — {fmtDate(dist.date)}
                      </span>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button className="btn btn-sm btn-ghost" onClick={() => setActivePrintId(null)}>
                          Close
                        </button>
                        <button className="btn btn-sm btn-primary" onClick={() => window.print()}>
                          🖨 Print
